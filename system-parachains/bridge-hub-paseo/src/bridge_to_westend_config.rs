...
//! Bridge definitions used for bridging with Westend Bridge Hub.

use crate::{
	weights,
	xcm_config::{UniversalLocation, XcmRouter},
	AccountId, Balance, Balances, BlockNumber, BridgeWestendMessages, PolkadotXcm, Runtime,
	RuntimeEvent, RuntimeOrigin, XcmOverBridgeHubWestend,
};
...
/// Lane identifier, used to connect Polkadot Asset Hub and Westend Asset Hub.
...
parameter_types! {
	...
	/// Polkadot -> Westend bridge message.
	///
...
parameter_types! {
	/// Westend Network identifier.
	pub WestendGlobalConsensusNetwork: NetworkId = NetworkId::Westend;
	/// Westend Network as `Location`.
	pub WestendGlobalConsensusNetworkLocation: Location = Location {
		parents: 2,
		interior: [GlobalConsensus(WestendGlobalConsensusNetwork::get())].into()
	};
	/// Interior location (relative to this runtime) of the with-Westend messages pallet.
	pub BridgePolkadotToWestendMessagesPalletInstance: InteriorLocation = PalletInstance(<BridgeWestendMessages as PalletInfoAccess>::index() as u8).into();
	...
	/// Identifier of the bridged Westend Asset Hub parachain.
	pub AssetHubWestendParaId: cumulus_primitives_core::ParaId = westend_runtime_constants::system_parachain::ASSET_HUB_ID.into();
	/// Location of the bridged Westend Bridge Hub parachain.
	pub BridgeHubWestendLocation: Location = Location {
		parents: 2,
		interior: [
	...
	/// A route (XCM location and bridge lane) that the Polkadot Asset Hub -> Westend Asset Hub
	/// message is following.
	pub FromAssetHubPolkadotToAssetHubWestendRoute: SenderAndLane = SenderAndLane::new(
		ParentThen(Parachain(AssetHubPolkadotParaId::get().into()).into()).into(),
		XCM_LANE_FOR_ASSET_HUB_POLKADOT_TO_ASSET_HUB_KUSAMA,
	);

	/// Lane identifier, used to connect Polkadot Asset Hub and Westend Asset Hub.
	pub const AssetHubPolkadotToAssetHubWestendMessagesLane: bp_messages::LaneId
		= XCM_LANE_FOR_ASSET_HUB_POLKADOT_TO_ASSET_HUB_KUSAMA;
	...
	pub ActiveLanes: sp_std::vec::Vec<(SenderAndLane, (NetworkId, InteriorLocation))> = sp_std::vec![
			(
				FromAssetHubPolkadotToAssetHubWestendRoute::get(),
...
parameter_types! {
	/// Number of Westend headers to keep in the runtime storage.
	///
	...
	/// Number of Westend Bridge Hub headers to keep in the runtime storage.
	///
	/// Note that we are keeping only required header information, not the whole header itself. Roughly, it
	/// is the 2 hours of real time (assuming that every header is submitted).
	pub const ParachainHeadsToKeep: u32 = 600;
	/// Maximal size of Westend Bridge Hub header **part** that we are storing in the runtime storage.
	pub const MaxParaHeadDataSize: u32 = bp_westend::MAX_NESTED_PARACHAIN_HEAD_DATA_SIZE;

	/// Bridge specific chain (network) identifier of the Westend Bridge Hub.
	pub const BridgeHubWestendChainId: bp_runtime::ChainId = bp_bridge_hub_westend::BridgeHubWestend::ID;
	/// Name of the `paras` pallet at Westend that tracks all parachain heads.
	pub const ParachainPalletNameAtWestend: &'static str = bp_westend::PARAS_PALLET_NAME;

	/// Maximal number of entries in the unrewarded relayers vector at the Polkadot Bridge Hub. It matches the
	/// maximal number of unrewarded relayers that the single confirmation transaction at Westend Bridge
	/// Hub may process.
	...
	/// uncinfirmed messages that the single confirmation transaction at Westend Bridge Hub may process.
...
pub type BridgeGrandpaWestendInstance = pallet_bridge_grandpa::Instance1;
...
pub type BridgeParachainWestendInstance = pallet_bridge_parachains::Instance1;
impl pallet_bridge_parachains::Config<BridgeParachainWestendInstance> for Runtime {
	...
	type ParasPalletName = ParachainPalletNameAtWestend;
...
/// Add XCM messages support for exchanging messages with BridgeHubWestend.
pub type WithBridgeHubWestendMessagesInstance = pallet_bridge_messages::Instance1;
...
/// Messages delivery proof for Polkadot Bridge Hub -> Westend Bridge Hub messages.
pub type ToWestendBridgeHubMessagesDeliveryProof =
	FromBridgedChainMessagesDeliveryProof<bp_bridge_hub_westend::Hash>;

/// Dispatches received XCM messages from Westend BridgeHub.
type FromWestendMessageBlobDispatcher = BridgeBlobDispatcher<
	XcmRouter,
	UniversalLocation,
	BridgePolkadotToWestendMessagesPalletInstance,
>;
...
/// Messaging Bridge configuration for BridgeHubPolkadot -> BridgeHubWestend
...
/// Maximal outbound payload size of BridgeHubPolkadot -> BridgeHubWestend messages.
...
pub type RefundBridgeHubWestendMessages = RefundSignedExtensionAdapter<
	RefundBridgedParachainMessages<
		Runtime,
		RefundableParachain<BridgeParachainWestendInstance, bp_bridge_hub_westend::BridgeHubWestend>,
		RefundableMessagesLane<
			WithBridgeHubWestendMessagesInstance,
			AssetHubPolkadotToAssetHubWestendMessagesLane,
		>,
		ActualFeeRefund<Runtime>,
		PriorityBoostPerMessage,
		StrRefundBridgeHubWestendMessages,
	>,
>;
bp_runtime::generate_static_str_provider!(RefundBridgeHubWestendMessages);
...
mod tests {
	...
	fn ensure_bridge_integrity() {
		...
		>(AssertCompleteBridgeConstants {
			...
			messages_pallet_constants: AssertBridgeMessagesPalletConstants {
				...
				max_unconfirmed_messages_in_bridged_confirmation_tx:
					bp_bridge_hub_westend::MAX_UNCONFIRMED_MESSAGES_IN_CONFIRMATION_TX,
				bridged_chain_id: bp_bridge_hub_westend::BridgeHubWestend::ID,
			},
			pallet_names: AssertBridgePalletNames {
				with_this_chain_messages_pallet_name:
					bp_bridge_hub_polkadot::WITH_BRIDGE_HUB_POLKADOT_MESSAGES_PALLET_NAME,
				with_bridged_chain_grandpa_pallet_name: bp_westend::WITH_KUSAMA_GRANDPA_PALLET_NAME,
				with_bridged_chain_messages_pallet_name:
					bp_bridge_hub_westend::WITH_BRIDGE_HUB_KUSAMA_MESSAGES_PALLET_NAME,
			},
		});
		...
		assert_eq!(
			BridgePolkadotToWestendMessagesPalletInstance::get(),
			Into::<InteriorLocation>::into(PalletInstance(
				bp_bridge_hub_polkadot::WITH_BRIDGE_POLKADOT_TO_KUSAMA_MESSAGES_PALLET_INDEX
			))
		);

		assert!(BridgeHubWestendLocation::get()
			.starts_with(&WestendGlobalConsensusNetworkLocation::get()));
	}
}
